	...
	function slider(){
		if($('.b-slider__list').length > 0){
			$('.b-slider__list').each(function () {
			    ...
			    if ($(this).hasClass("b-slider__list--main"))
					auto_start = true;
				else
					auto_start = false;
...
//			$('.b-header__searchInput').val('');
	...
	$('.b-slider__optionList').on('click','a',function(e){
        ...
        if(class_.trim()=='b-slider__optionBasket')
        {
        ...
        if(class_.trim()=='b-slider__optionBasket active')
        {
            return false;
        }
	...
	$(document).click(function(e) {
		if($(e.target).closest(".js-menuButton").length ==0 &&
			...
			(!$('.js-menuButton').hasClass('.js-index') && $(window).width() >= '991')){
...
//	console.log(costMax);
	...
	$("#datapicker").attr("placeholder", "Выберите дату");
	...
	function textCut(){
		$('.b-slider__list--last .b-slider__title, .b-slider__newsText').textTailor({
			minFont: 9,
