...
*/
    ...
    $new_text =
        '<nav class="b-item__content">
            <div class="items-wrap" id="products_ul">
				<div class="list" data-screen="screenItem" id="products_ul">'.
        		$new_text .
        	'</div></div>
     </nav>';
...
while(strpos($text, '[komplekt')>0)
{
	...
	{
		while(strpos($text, '[rating=')>0)
			$text = text_rating_replace($text);

		$text .= '<link href="/templates/pianino_new/css/new.css" rel="stylesheet" type="text/css" />';
	}
