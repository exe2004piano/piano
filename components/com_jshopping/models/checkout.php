...
class jshopCheckout{
    ...
    function sendOrderEmail($order_id, $manuallysend = 0){
		...
		$order->currency_code = "руб.";
		$order->currency_code_iso = "BYR";

		$currency = JTable::getInstance('currency', 'jshop');
		$currency->load(2);
		$curs = $currency->currency_value;

		$order->order_total = $order->order_total * $curs;
		$order->order_subtotal = $order->order_subtotal * $curs;
		$order->order_tax = $order->order_tax * $curs;
		foreach($order->items AS $i)
		{
			$i->product_item_price = $i->product_item_price * $curs;
			$i->product_tax = $i->product_tax * $curs;
		}
        ...
        if ($admin_send_order){
			...
			$message_admin = str_replace('<td colspan="5" class="bg_gray">', '<td colspan="5" class="bg_gray">PROMOCODE: ' . $_COOKIE['roistat_visit'] . "NEW Code: " . $_COOKIE['roi_new'] . "<br /><br />\n", $message_admin);
