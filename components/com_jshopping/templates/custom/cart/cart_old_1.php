...
<table class="jshop cart" border="1">
  ...
  <tr class="jshop_prod_cart <?php  print $class; if ($i%2==0) print "even"; else print "odd"?>">
    ...
    <td style="text-align: center; vertical-align: middle;">
      <input type = "text" name = "quantity[<?php print $key_id ?>]" value = "<?php print $prod['quantity'] ?>" class = "inputbox" style = "width: 25px" onchange="document.updateCart.submit();" />
...
<!--      <span class = "cart_reload"><img style="cursor:pointer" src="<?php print $this->image_path ?>/images/reload.png" title="<?php print _JSHOP_UPDATE_CART ?>" alt = "<?php print _JSHOP_UPDATE_CART ?>" onclick="document.updateCart.submit();" /></span> -->
    ...
    <td>
        <?php print number_format(formatprice($prod['price'])*$currency->currency_value*$prod['quantity'], 0, " ", " ").' '.$currency->currency_code; ?>
...
<table class="jshop jshop_subtotal">
  ...
  <?php }?>
    ...
    <input type="text" placeholder="Ваше имя" name="basket_user_name" id="basket_user_name" /><br />
    <input type="text" placeholder="Номер телефона (+375-29-ххххххх)" name="basket_user_phone" id="basket_user_phone" /><br />
    <input type="text" placeholder="Адрес доставки" name="basket_user_adr" id="basket_user_adr" /><br />
    <input type="text" placeholder="Ваш email" name="basket_user_email" id="basket_user_email" /><br />
    <textarea placeholder="Ваш комментарий к заказу" name="basket_user_comment" id="basket_user_comment"></textarea><br />
