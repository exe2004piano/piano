<?php
	...
	$cat_list = "";
	$db->setQuery("
SELECT c.*, c.`name_ru-RU` title, cp.`name_ru-RU` cp_title
...
");
    ...
    */
	...
	gtag('event', 'Ecomm Product ID', {'ecomm_prodid':".$product->product_id."}); 
	...
	switch($product->sklad)
	{
		case '0' :
			...
			$dost_srok_minsk = $cat_temp->sklad_0_minsk;
			$dost_srok_rb = $cat_temp->sklad_0_rb;
			break;
		case '1' :
			$sklad_title = 'на складе';
			$sklad_status = 'inStock';
			$dost_srok_minsk = $cat_temp->sklad_1_minsk;
			$dost_srok_rb = $cat_temp->sklad_1_rb;
			break;
		case '2' :
		...
		case '4' :
			...
			$sklad_status = 'order';
			//$dost_srok_minsk = $cat_temp->sklad_4_minsk;
			//$dost_srok_rb = $cat_temp->sklad_4_rb;
