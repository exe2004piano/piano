...
class jshopProduct extends JTableAvto{
    ...
    */
    ...
    function getRelatedProducts(){
        ...
        $query = "SELECT $adv_result, prod.sklad sklad, prod.cz cz,prod.komplekts komplekts,
                prod.tab_1, prod.tab_2, prod.tab_3, prod.tab_4
    ...
    function getLastProducts($count, $array_categories = null, $filters = array()){
        ...
        $query = "SELECT $adv_result, sklad, cz FROM `#__jshopping_products` AS prod
    ...
    function getReviews($limitstart = 0, $limit = 20) {
        $query = "SELECT * FROM `#__jshopping_products_reviews` WHERE product_id = '".$this->_db->escape($this->product_id)."' and publish='1' order by `time` desc";
        $this->_db->setQuery($query, $limitstart, $limit);
        return $this->_db->loadObjectList();
    }
