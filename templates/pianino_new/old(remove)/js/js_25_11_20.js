...
{
    $(".z-form").unbind("submit");
    $(".z-form").on("submit", function(e)
    {
        e.preventDefault();
        var z_form = $(this).serialize();
        $.ajax({
            type: 'POST',
            url: '/exe/send_form.php',
            data: z_form,
            success: function(json)
            {
                // location.href = location.href+"?ok#thanks";
                $(".pop__up").removeClass('is--show');
...
{
    $(".cat_link").unbind("click");
    $(".cat_link").on("click", function(e)
    {
        e.preventDefault();
        var cat = $(this).data("cat");
        $(".cat_link").removeClass('is--active');
