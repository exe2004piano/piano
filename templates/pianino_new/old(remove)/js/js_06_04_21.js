...
function update_show_more_review_click()
{
    $(".show_more_reviews").unbind("click");
    $(".show_more_reviews").on("click", function(e)
    {
        e.preventDefault();
        var product_id = $(this).data('id');
        var product_p = $(this).data('p');
        var mark = $("[name='product_mark_select']:checked").data("mark");

        $.get("/z/get_product_reviews.php", {product_id:product_id, product_p:product_p, mark:mark}, function(data)
        {
            $(".reviews-blocks").html(data);
            update_show_more_review_click();
            rev_base_64();
        });
    });
}
...
jQuery(document).ready(function ($)
{
    ...
    $(".rating__checkbox-label").on("click", function()
    {
        setTimeout(function()
        {
            $(".show_more_reviews").trigger("click");
        }, 100);
    });
