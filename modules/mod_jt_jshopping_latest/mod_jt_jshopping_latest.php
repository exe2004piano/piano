...
defined('_JEXEC') or die;
	...
	$document->addStyleSheet(JURI::base() . 'modules/mod_jt_jshopping_latest/css/style.css');
	...
	if ($jt_load_jquery == 0) {
		...
		if ($jquery == 1) { $document->addScript(JURI::base() . 'modules/mod_jt_jshopping_latest/js/jquery.min.js'); }
		if ($jt_script_bx == 1 ) { $document->addScript(JURI::base() . 'modules/mod_jt_jshopping_latest/js/jquery.bxSlider.min.js'); }
	...
	if ($jt_load_jquery == 1) { 
		...
		if ($jquery == 1) { $document->addCustomTag('<script type = "text/javascript" src = "'.JURI::root().'modules/mod_jt_jshopping_latest/js/jquery.min.js"></script>'); }
		if ($jt_script_bx == 1 ) { $document->addCustomTag('<script type = "text/javascript" src = "'.JURI::root().'modules/mod_jt_jshopping_latest/js/jquery.bxSlider.min.js"></script>');	}
		$document->addCustomTag('<script type = "text/javascript">if (jQuery) jQuery.noConflict();</script>');	
		}
	require JModuleHelper::getLayoutPath('mod_jt_jshopping_latest', $params->get('layout', 'default'));
	echo JText::_(MOD_JT);
?>