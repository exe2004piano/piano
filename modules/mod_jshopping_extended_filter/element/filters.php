	...
	function fetchElement($name, $value, &$node, $control_name) {
			...
			if($list) {
				...
				foreach ( $list as $item ) {
					if(is_object($item)) {
                        ...
                        $mitems[sizeof($mitems)-1]->text = "(".$min_cat.") ".$mitems[sizeof($mitems)-1]->text;
					}
					else {
			...
			<script type='text/javascript'>
				...
				if(typeof jQuery == 'undefined') {
					...
					script.type = 'text/javascript';
					script.src = '".JURI::root(true)."/modules/mod_jshopping_extended_filter/assets/js/jquery-1.8.2.min.js';
				...
				function load_ui() {
					if(typeof jQuery.ui == 'undefined') {
					   var script = document.createElement('script');
					   script.type = 'text/javascript';
					   script.src = FilterPath+'js/jquery-ui-1.9.1.custom.min.js';
					   document.getElementsByTagName('head')[0].appendChild(script);

					   var style = document.createElement('link');
					   style.rel = 'stylesheet';
					   style.type = 'text/css';
					   style.href = FilterPath+'css/smoothness/jquery-ui-1.9.1.custom.css';
